...
const state = {
    ...
    selectedTopics: [],     // 已選擇的章節（空陣列 = 全部）
    availableTopics: [],    // 當前題庫可用的章節列表
    settings: {
...
const els = {
    ...
    // 章節選擇器
    topicSelectorSection: $('topic-selector-section'),
    topicDropdown: $('topic-dropdown'),
    topicDropdownBtn: $('topic-dropdown-btn'),
    topicDropdownMenu: $('topic-dropdown-menu'),
    topicSelectedText: $('topic-selected-text'),
    topicQuestionCount: $('topic-question-count'),
    // 測驗
...
// 從題庫中提取所有不重複的 topic，並排序
function getTopicsFromBank(bank) {
    const topicMap = new Map();
    bank.questions.forEach(q => {
        if (q.topic !== undefined && q.topic !== null) {
            const count = topicMap.get(q.topic) || 0;
            topicMap.set(q.topic, count + 1);
        }
    });
    // 轉換為陣列並排序
...
function renderTopicSelector() {
    if (!state.currentBank) {
        els.topicSelectorSection?.classList.add('hidden');
        return;
    }

    const topics = getTopicsFromBank(state.currentBank);
    state.availableTopics = topics;

    // 如果題庫沒有 topic 欄位，隱藏選擇器
    if (topics.length === 0) {
        els.topicSelectorSection?.classList.add('hidden');
        return;
    }
    ...
    // 顯示選擇器
    els.topicSelectorSection?.classList.remove('hidden');

    // 重置選擇狀態
    state.selectedTopics = [];

    // 建立選項 HTML
    const totalCount = state.currentBank.count;
    let optionsHtml = `
        <div class="topic-option select-all selected" data-topic="all">
            <div class="topic-option-checkbox"></div>
            <span class="topic-option-label">全部章節</span>
            <span class="topic-option-count">${totalCount} 題</span>
        </div>
    `;

    topics.forEach(({ topic, count }) => {
        optionsHtml += `
            <div class="topic-option" data-topic="${topic}">
                <div class="topic-option-checkbox"></div>
                <span class="topic-option-label">Topic ${topic}</span>
                <span class="topic-option-count">${count} 題</span>
            </div>
        `;
    });

    if (els.topicDropdownMenu) {
        els.topicDropdownMenu.innerHTML = optionsHtml;
        // 綁定選項點擊事件
        els.topicDropdownMenu.querySelectorAll('.topic-option').forEach(opt => {
            opt.onclick = (e) => {
                e.stopPropagation();
                ...
                if (topic === 'all') {
                    ...
                    toggleTopicSelection(parseInt(topic));
                }
            };
        });
    }

    updateTopicSelectedText();
    updateTopicQuestionCount();
}

// 切換下拉選單開關
function toggleTopicDropdown() {
    els.topicDropdown?.classList.toggle('open');
    els.topicDropdownMenu?.classList.toggle('hidden');
}

// 關閉下拉選單
function closeTopicDropdown() {
    els.topicDropdown?.classList.remove('open');
    els.topicDropdownMenu?.classList.add('hidden');
}

// 選擇全部章節
function selectAllTopics() {
    state.selectedTopics = [];
    updateTopicOptionsUI();
    updateTopicSelectedText();
    updateTopicQuestionCount();
}
...
// 更新選項的選取狀態 UI
function updateTopicOptionsUI() {
    if (!els.topicDropdownMenu) return;

    const isAllSelected = state.selectedTopics.length === 0;

    els.topicDropdownMenu.querySelectorAll('.topic-option').forEach(opt => {
        const topic = opt.dataset.topic;
        if (topic === 'all') {
            opt.classList.toggle('selected', isAllSelected);
        } else {
            const topicNum = parseInt(topic);
            opt.classList.toggle('selected', state.selectedTopics.includes(topicNum));
        }
    });
}

// 更新下拉按鈕顯示的選中文字
function updateTopicSelectedText() {
    if (!els.topicSelectedText) return;

    if (state.selectedTopics.length === 0) {
        els.topicSelectedText.textContent = '全部章節';
    } else if (state.selectedTopics.length === 1) {
        els.topicSelectedText.textContent = `Topic ${state.selectedTopics[0]}`;
    } else if (state.selectedTopics.length <= 3) {
        els.topicSelectedText.textContent = state.selectedTopics.map(t => `T${t}`).join(', ');
    } else {
        els.topicSelectedText.textContent = `已選 ${state.selectedTopics.length} 個章節`;
    }
}

// 更新篩選後的題目數量顯示
function updateTopicQuestionCount() {
    if (!els.topicQuestionCount || !state.currentBank) return;

    const filteredCount = getFilteredQuestionCount();
    els.topicQuestionCount.textContent = `${filteredCount} 題可練習`;

    // 同時更新自訂數量的最大值
    if (els.customCount) {
        els.customCount.max = filteredCount;
    }
}

// 取得篩選後的題目數量
function getFilteredQuestionCount() {
    if (!state.currentBank) return 0;
    if (state.selectedTopics.length === 0) return state.currentBank.count;

    return state.currentBank.questions.filter(q =>
        state.selectedTopics.includes(q.topic)
    ).length;
}

// 根據選擇的章節過濾題目
function filterQuestionsByTopics(questions) {
    if (state.selectedTopics.length === 0) return questions;
    return questions.filter(q => state.selectedTopics.includes(q.topic));
}
...
function startQuiz() {
    ...
    // 準備題目：先套用章節過濾
    let questions = filterQuestionsByTopics([...state.currentBank.questions]);
...
function startWrongQuestionsPractice() {
    ...
    // 從原題庫找出錯題（使用 questionKey 比較），並套用章節過濾
    ...
    // 套用章節過濾
    questions = filterQuestionsByTopics(questions);
...
function initEvents() {
    ...
    // 章節選擇器下拉選單
    if (els.topicDropdownBtn) {
        els.topicDropdownBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            toggleTopicDropdown();
        });
    }
    ...
    document.addEventListener('click', (e) => {
        if (els.topicDropdown && !els.topicDropdown.contains(e.target)) {
            closeTopicDropdown();
        }
    });
